...
export async function createPlatformWindow({
	...
	window.webContents.on("render-process-gone", (_event, details) => {
		...
		// Fork-local (5c5753519): recover from real crashes/aborts. `clean-exit`
		// is shutdown; `killed` is intentional (e.g. via task manager) and the
		// user can reopen the window themselves.
