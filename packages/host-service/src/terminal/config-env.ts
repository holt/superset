/**
 * Config env resolver for v2 host-service terminals.
 * NOTE: A parallel implementation exists at apps/desktop/src/main/lib/terminal/config-env.ts
 * for desktop daemon-backed terminals. Keep both in sync when making changes.
 */
...
function readEnvFromConfig(configPath: string): Record<string, EnvValue> | undefined {
 ...
 * a flat Record<string, string> ready to merge into the PTY environment.
 *
...
export async function resolveConfigEnv({
	...
	// Worktree config takes precedence, merge with main repo
	const mainEnv = readEnvFromConfig(
		join(mainRepoPath, PROJECT_SUPERSET_DIR, CONFIG_FILE),
	);
	const worktreeEnv =
		worktreePath !== mainRepoPath
			? readEnvFromConfig(join(worktreePath, PROJECT_SUPERSET_DIR, CONFIG_FILE))
			: undefined;

	const merged = mainEnv || worktreeEnv
		? { ...mainEnv, ...worktreeEnv }
		: undefined;
